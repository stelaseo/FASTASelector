...
namespace FASTASelector.Data
{
    ...
    {
        private sealed class TaskSequenceRead : Task
        {
            ...
            public override bool Initialize( Controller controller )
            {
                ...
                _newList = new SequenceCollection( );
                if( _newList.Read( FileName ) )
                {
                    App.Log( App.DEFAULT, "Total of {0} sequence(s) have been read from {1}", _newList.Count, FileName );
            ...
            {
                App.Log( App.DEFAULT, "Reverted the sequence list to the state prior to reading {0}", FileName );
